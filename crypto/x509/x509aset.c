...
void X509_ACERT_set0_holder_baseCertId(X509_ACERT *x,
                                       ISSUER_SERIAL *isss)
{
    ISSUER_SERIAL_free(x->acinfo->holder.baseCertificateID);
...
{
    OBJECT_DIGEST_INFO_free(x->acinfo->holder.objectDigestInfo);
    x->acinfo->holder.objectDigestInfo = dinfo;
}

int X509_ACERT_set_issuerName(X509_ACERT *x, const X509_NAME *name)
{
    ...
    if (v2Form == NULL) {
        v2Form = ACERT_ISSUER_V2FORM_new();
        if (v2Form == NULL)
            goto oom;
        x->acinfo->issuer.u.v2Form = v2Form;
        x->acinfo->issuer.type = ACERT_ISSUER_V2;
    }
...
int X509_ACERT_set_serialNumber(X509_ACERT *x, ASN1_INTEGER *serial)
{
