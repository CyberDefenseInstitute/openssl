                        ...
                        unsigned long cflag)
{
    ...
    if (!(cflag & X509_FLAG_NO_SUBJECT)) {
        ...
        if ((holder_bcid = X509_ACERT_get0_holder_baseCertId(x)) != NULL)
            holderIssuer = ISSUER_SERIAL_get0_issuer(holder_bcid);
        ...
        if (holderIssuer != NULL) {
            ...
            BIGNUM *serial_bn;
            holder_serial = ISSUER_SERIAL_get0_serial(holder_bcid);
            ...
            serial_bn = BN_new();
            serial_str = BN_bn2hex(ASN1_INTEGER_to_BN(holder_serial, serial_bn));
            BN_free(serial_bn);
            ...
            iuid = ISSUER_SERIAL_get0_issuerUID(holder_bcid);
            if (iuid != NULL) {
    ...
    if (!(cflag & X509_FLAG_NO_ATTRIBUTES)) {
        ...
        if (X509_ACERT_get_attr_count(x) == 0) {
            ...
            for (i = 0; i < X509_ACERT_get_attr_count(x); i++) {
                if (!print_attribute(bp, X509_ACERT_get_attr(x, i)))
                    goto err;
            }
        }
    }
